    ...
    public class LoanCalculatorService : ILoanCalculatorService
    {
        ...
        public string CalculateLoanPayment(LoanApplicationView model)
        {
            ...
            try
            {
                ...
                RepaymentFrequencies repayfrequency = Enum.Parse<RepaymentFrequencies>(model.RepaymentFrequency.Replace(' ', '_'));
        ...
        public string CalculateLoanPayment2(LoanApplicationView model)
        {
            try
            {
                //get input from the API Model
                double loanAmount = model.LoanAmount;
                ...
                double interestRatePerPeriod = interestRate / (12 * 100);
                Repayment[] repayments = new Repayment[(int)loanTerm];

                for (int i = 0; i < (int)loanTerm; i++)
                {
                    Installment[] installments = new Installment[(int)repaymentsPerMonth];
                    var sstartDate = new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1).AddMonths(i);
                    interestPayment = balance * interestRatePerPeriod;
                    principalPayment = (loanAmount / loanTerm) + (loanAmount / loanTerm) * (interestRatePerPeriod * i);
                    balance = balance - (loanAmount / loanTerm);
                    totalInterest += interestPayment;
                    totalAmount += interestPayment + (loanAmount / loanTerm);

                    repayments[i] = new Repayment
                    {
                        RepaymentMonth = sstartDate.ToString("MMMM yyyy"),
                        Interest = RoundUp(interestPayment, 2),
                        RepaymentAmount = RoundUp((loanAmount / loanTerm), 2),
                        RemainingBalance = RoundUp(balance, 2),
                        Principal = RoundUp((loanAmount / loanTerm) - interestPayment, 2),
                        Installments = installments,
                    };
                    ...
                    for (int x = 0; x < (int)repaymentsPerMonth; x++)
                    {
                        ...
                        {
                            InstallmentId = x + 1,
                            AmountPayable = RoundUp((loanAmount / loanTerm) / (int)repaymentsPerMonth, 2),
                        };
                    }
                }
                var result = new LoanRepaymentView
